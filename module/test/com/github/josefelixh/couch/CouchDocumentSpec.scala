...
class CouchDocumentSpec extends FlatSpec with MustMatchers with MockitoSugar {
  ...
  "A CouchDocument object" should "persist the document when calling create" in new CouchMock {
    val couchDocument = CouchDocument(jsonTestObject)

    val responseMock = mock[Response]
    stub(responseMock.json).toReturn(jsonTestObjectSuccessResponse)
    when(requestMock.post(jsonTestObject)(CouchDocument.wrt, CouchDocument.cto)).thenReturn(future { responseMock })

    val createdCouchDocument = Await.result(couchDocument.create, 0.1 seconds)

    createdCouchDocument.id.get must be === "test_id"
    createdCouchDocument.rev.get must be === "test_rev"
    createdCouchDocument.doc.get must be === jsonTestObject

    verify(requestMock, times(1)).post(jsonTestObject)(CouchDocument.wrt, CouchDocument.cto)
  ...
  it should "update the document when calling update" in new CouchMock {
    val responseMock = mock[Response]
    stub(responseMock.json).toReturn(jsonTestObjectSuccessResponse)
    when(requestMock.put(updatedJsonTestObject.toString())(couch.wrt, couch.cto)).thenReturn(future { responseMock })
